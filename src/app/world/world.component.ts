...
export class WorldComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit(): void {
    ...
    const url = decodeURIComponent(
      this.worldService.getBaseUrl(this.world.owner, this.world.name),
    );
    ...
    const description =
      this.world.description !== undefined && this.world.description !== ""
  ...
  public downloadClick(): void {
    this.worldService.download(this.world, this.currentVersion).subscribe(
      (file) => {
        FileSaver.saveAs(file, `${this.world.name}.zip`);

        // Refresh the World.
        this.worldService
          .get(this.world.owner, this.world.name)
          .subscribe((world) => {
            this.world = world;
            this.getFiles();
          });
  ...
  public downloadIndividualFile(file: File): void {
    const url = this.worldService.getIndividualFileUrl(
      this.world,
      file,
      this.currentVersion,
    );
  ...
  public copyWorld(): void {
    ...
    this.copyNameDialog.componentInstance.onSubmit.subscribe((result) => {
      if (result !== undefined && result.copyName.trim() !== "") {
        ...
        this.worldService
          .copy(this.world, result.copyName.trim(), result.copyOwner)
          .subscribe(
            (response) => {
              ...
              this.router.navigate([
                `/${response.owner}/worlds/${response.name}`,
              ]);
            },
            (error) => {
              this.snackBar.open(error.message, "Got it");
  ...
  public report(): void {
    ...
    this.reportWorldDialog = this.dialog.open(
      ReportDialogComponent,
      dialogOptions,
    );

    this.reportWorldDialog.componentInstance.onSubmit.subscribe((result) => {
  ...
  public addToCollection(): void {
    ...
    this.collectionDialogSubscription = this.collectionDialog
      .afterClosed()
      .subscribe((result) => {
        if (result === true) {
  ...
  public getFiles(): void {
    this.worldService
      .getFileTree(this.world, this.currentVersion)
      .pipe(
        finalize(() => {
  ...
  public onVersion(): void {
    ...
    this.getFiles();
    this.location.go(
      `${this.world.owner}/worlds/${this.world.name}/${this.currentVersion}`,
    );
  }
