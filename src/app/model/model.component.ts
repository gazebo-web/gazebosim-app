...
export class ModelComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit(): void {
    ...
    const description =
      this.model.description !== undefined && this.model.description !== ""
  ...
  public downloadClick(): void {
    this.modelService.download(this.model, this.currentVersion).subscribe(
      (file) => {
        FileSaver.saveAs(file, `${this.model.name}.zip`);

        // Refresh the Model.
        this.modelService
          .get(this.model.owner, this.model.name)
          .subscribe((model) => {
            this.model = model;
            this.getFiles();
          });
  ...
  public downloadIndividualFile(file: File): void {
    const url = this.modelService.getIndividualFileUrl(
      this.model,
      file,
      this.currentVersion,
    );
  ...
  public copyModel(): void {
    ...
    this.copyNameDialog.componentInstance.onSubmit.subscribe((result) => {
      if (result !== undefined && result.copyName.trim() !== "") {
        ...
        this.modelService
          .copy(this.model, result.copyName.trim(), result.copyOwner)
          .subscribe(
            (response) => {
              ...
              this.router.navigate([
                `/${response.owner}/models/${response.name}`,
              ]);
            },
            (error) => {
              this.snackBar.open(error.message, "Got it");
  ...
  public getFiles(): void {
    this.modelService
      .getFileTree(this.model, this.currentVersion)
      .pipe(
        finalize(() => {
  ...
  public report(): void {
    ...
    this.reportModelDialog = this.dialog.open(
      ReportDialogComponent,
      dialogOptions,
    );

    this.reportModelDialog.componentInstance.onSubmit.subscribe((result) => {
  ...
  public onVersion(): void {
    ...
    this.getFiles();
    this.location.go(
      `${this.model.owner}/models/${this.model.name}/${this.currentVersion}`,
    );
  }
