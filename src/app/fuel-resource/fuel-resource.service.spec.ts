...
describe("FuelResourceService", () => {
  ...
  it(`should get a list of resources`, () => {
    ...
    const search = "test";
    url += `?q=${search}`;
    service.getList({ search: search }).subscribe((response) => {
      expect(response.resources[0].name).toBe(testResource.name);
    });
  ...
  it(`should get a list of resources owned by someone`, () => {
    ...
    service.getOwnerList(user).subscribe((response) => {
      expect(response.resources[0].name).toBe(testResource.name);
    });
  ...
  it(`should get a list of resources liked by someone`, () => {
    ...
    service.getUserLikedList(user).subscribe((response) => {
      expect(response.resources[0].name).toBe(testResource.name);
    });
  ...
  it(`should get the next page of a paginated resource`, () => {
    ...
    service
      .getNextPage({ nextPage: "nextPageUrl" } as FuelPaginatedResource)
      .subscribe((response) => {
        expect(response.resources[0].name).toBe(testResource.name);
      });
  ...
  it(`should get a single resource`, () => {
    ...
    service.get(user, name).subscribe((response) => {
  ...
  it(`should edit a resource`, () => {
    const url = `${service.baseUrl}/${user}/${service.resourceType}/${name}`;
    service.edit(user, name, formData).subscribe((response) => {
      expect(response.name).toBe(testResource.name);
    });
  ...
  it(`should delete a resource`, () => {
    const url = `${service.baseUrl}/${user}/${service.resourceType}/${name}`;
    service
      .delete({ owner: user, name } as FuelResource)
      .subscribe((response) => {
        expect(response.name).toBe(testResource.name);
      });
  ...
  it(`should like a resource`, () => {
    ...
    service.like(testResource).subscribe((response) => {
  ...
  it(`should unlike a resource`, () => {
    const url = `${service.baseUrl}/${user}/${service.resourceType}/${name}/likes`;
    service.unlike(testResource).subscribe((response) => {
  ...
  it(`should copy a resource`, () => {
    ...
    service.copy(testResource, "newName", "newOwner").subscribe((response) => {
      expect(response.name).toBe(testResource.name);
    });
  ...
  it(`should download a resource as a Zip file`, () => {
    ...
    service.download(testResource).subscribe((response) => {
    ...
    service.download(testResource, version).subscribe((response) => {
      expect(response).toEqual(new Blob([]));
    });
  ...
  it(`should get a file as a blob`, () => {
    ...
    service.getFileAsBlob(url).subscribe((response) => {
      expect(response).toEqual(new Blob([]));
    });
  ...
  it(`should handle the link header`, () => {
    const header: HttpHeaders = new HttpHeaders({
      link: '</test-next?page=1>; rel="next"',
    });
