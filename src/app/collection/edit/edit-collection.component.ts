...
export class EditCollectionComponent implements OnInit {
  ...
  public onEdit(): void {
    ...
    this.collectionService
      .editCollection(this.collection.owner, this.collection.name, formData)
      .subscribe(
        (response) => {
          this.snackBar.open(
          ...
          this.router.navigate([
            `${this.collection.owner}/collections/${this.collection.name}`,
          ]);
        },
        (error) => {
  ...
  public onDelete(): void {
    const dialogOps = {
      data: {
        ...
        message:
          "Be mindful when deleting a model, other users could be counting on it.",
        buttonText: "Delete",
        hasInput: true,
    ...
    this.confirmationDialog.afterClosed().subscribe((result) => {
      ...
      this.collectionService
        ...
        .subscribe(
          (response) => {
            this.snackBar.open(
              this.collection.name + " collection deleted",
              "Got it",
              {
                duration: 2750,
              },
